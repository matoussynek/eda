...
public class GrafoRedMunicipios {
    ...
    public GrafoRedMunicipios(String nomFich) {
        ...
        adyKruskal = new TablaHash<Municipio, ListaConPI<Municipio>>(MAX_MUNICIPIOS);
    ...
    public double crearAdyKruskal() {      
        ...
        for(int i = 0; i < aristas.length; i++) {
            Arista ar = aristas[i];
            ListaConPI<Municipio> l = adyKruskal.recuperar(verticesAMunicipios.recuperar(ar.getOrigen()));
            if(l == null) {
                l = new LEGListaConPI<Municipio>();
                adyKruskal.insertar(verticesAMunicipios.recuperar(ar.getOrigen()), l);
            }
            l.insertar(verticesAMunicipios.recuperar(ar.getDestino()));
