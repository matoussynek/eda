...
public class ABB<E extends Comparable<E>> {
    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        ...
        int m = (ini+fin) / 2;
        NodoABB<E> root = new NodoABB<E>(v[m]);
        root.izq = construirEquilibrado(v, ini, m-1);
        root.der = construirEquilibrado(v, m+1, fin);
        root.talla = 1 + talla(root.izq) + talla(root.der);
        return root;
    }
    ...
    public void reconstruirEquilibrado() {
        ...
        // COMPLETAR
        E[] aux = toArrayInOrden();
        raiz = construirEquilibrado(aux, 0, aux.length-1);
    }
    ...
    protected NodoABB<E> sucesor(E e, NodoABB<E> actual) {
        ...
        int r = e.compareTo(actual.dato);
        if(r < 0) {
            NodoABB<E> x = sucesor(e, actual.izq);
            if(x == null) return actual;
            else return x;
        } else {
            return sucesor(e, actual.der);
        } 
}
