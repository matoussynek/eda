...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    public ListaConPI<C> claves() {
        //COMPLETAR
        LEGListaConPI<C> res = new LEGListaConPI<C>();
        for(int i = 0; i < elArray.length; i++){
    ...
    protected final void rehashing() {
        ...
        int newArraySize = siguientePrimo(2*elArray.length);
        this.elArray = new LEGListaConPI[newArraySize];
        ...
        for(int i = 0; i < elArrayAntiguo.length;i++){
            ListaConPI<EntradaHash<C, V>> cubeta = elArrayAntiguo[i];
            for(cubeta.inicio();!cubeta.esFin();cubeta.siguiente()){
                EntradaHash<C,V> e = cubeta.recuperar();
    ...
    public final double desviacionTipica() {
        ...
        double res = 0;
        for(int i = 0; i<elArray.length; i++){
            sumatorio += elArray[i].talla();
        }
        longitudMedia = sumatorio / elArray.length;
        sumatorio = 0;
        for(int i = 0; i<elArray.length; i++){
            double temp = elArray[i].talla()-longitudMedia;
            sumatorio += temp*temp;
        }
        res = Math.sqrt(sumatorio/elArray.length);
        return res;
        /*
         * double lMedia = factorCarga();
         * double suma = 0.0;
         * for(int i = 0; elArray.length; i++){ç
         *      double di = elArray[i].talla() - lMedia;
         *      suma += di *di;
         * }
