...
public class Recomendacion {
    ...
    public ReferenciasWeb maxReferencias() {
        if(noPags==0){return null;}
        ReferenciasWeb temp = (ReferenciasWeb) qp.eliminarMin();
        noPags--;
        return temp;
    }
    ...
    public ReferenciasWeb[] maxKReferencias(int k) {
        int size = Math.min(k,noPags);
        ReferenciasWeb [] res = new ReferenciasWeb [size];
    ...
    public String toString() {
        ReferenciasWeb aux=null;
        ColaPrioridad<ReferenciasWeb> temp = qp;
        String res ="";
        while(!temp.esVacia())
        {
            res+=temp.eliminarMin().toString();
        }
