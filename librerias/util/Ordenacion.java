    ...
    */
    ...
    int i, int f) {
        if(i==f){
            ...
            int m=(i+f)/2;
            T[] aux1=mergeSort2(v,i,m);
            T[] aux2=mergeSort2(v,m+1,f);
            return merge2(aux1,aux2);
    ...
    */
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {
        ...
        while(a<v1.length && b<v2.length){
            if(v1[a].compareTo(v2[b])<0){aux[c++]=v1[a++];}
            else{
                aux[c++]=v2[b++];
            }
        }
        while(a<v1.length){aux[c++]=v1[a++];}
        while(b<v2.length){aux[c++]=v2[b++];}
